...
class Camera_Calibration:
    ...
    def detect_checker_corners(self, image_files, verify_corners=False, custom_pts_path=None):
        ...
        for filename in image_files:
            ...
            if ret == True:
                ...
                #some detections have checkerboard points flipped. The code below will fix those points
                if corners[0][0][0]-corners[3][0][0] > 0:
                        corners = corners[::-1]

                if np.abs(corners[0][0][0]-corners[5][0][0]) < 60:
    ...
    def check_undistortion(self, cam_matrix, dist, distorted_imgs, path_to_save_undistorted, save_undistorted_img=True):
        ...
        for img_name in distorted_imgs:
            ...
            if save_undistorted_img == True:
                ...
                op_file_name = path_to_save_undistorted  + folder_name + img_name[uscore_id-4:][:-4] + "_undistorted.png"
                print(op_file_name)
    ...
    def stereo_calibrate(self,image_folder0,image_folder1, output_file_name='calibration_result.pickle', check_undistortion=False, custom_pts_filename = None):
        ...
        distortion_0 = np.array([-0.39084258,  0.11035878, -0.00997172, -0.01113429,  0.30664022]) #using distortion from stacy output
        ...
        distortion_1 = np.array([-1.73112330e-01, -2.02276643e+00,  2.64479994e-03,  5.55261622e-03, 9.16707953e+00]) #using distortion from stacy output
    ...
    def triangulate_pts(self, projection_mat_0, projection_mat_1, img_pts_0, img_pts_1, apply_distortion=False, frame_0=None, frame_1=None, show_triangulation_result=True, verbose= False):
        ...
        if apply_distortion == True:
            ...
            for uv0, uv1 in zip(img_pts_0, img_pts_1):
                u_uv0 = cv2.undistortPoints(uv0, cam_matrix_0, dist_c0)
                undist_img_pts_0.append(u_uv0[0][0])

                u_uv1 = cv2.undistortPoints(uv1, cam_matrix_1, dist_c1)
                undist_img_pts_1.append(u_uv1[0][0])
            
            undist_img_pts_0 = np.array(undist_img_pts_0).astype(np.float64)
            undist_img_pts_1 = np.array(undist_img_pts_1).astype(np.float64)
            assert undist_img_pts_0.shape == img_pts_0.shape
            img_pts_0 = undist_img_pts_0
            img_pts_1 = undist_img_pts_1
...
def test_calibration():
    ...
    calib_cam.stereo_calibrate(image_folder0='20110110-tm1-ub-maca/Calibration/Extrinseque/cam0', image_folder1='20110110-tm1-ub-maca/Calibration/Extrinseque/cam1', check_undistortion=True, custom_pts_filename= "manual_pts_data.pkl" )
    ...
    p3ds, dist3d_list = calib_cam.triangulate_pts(projection_mat_0=P0, projection_mat_1=P1, img_pts_0=uv_0, img_pts_1=uv_1, apply_distortion=False, frame_0=frame_0, frame_1=frame_1, show_triangulation_result=True, verbose=True)
...
#stacy distortion output
# cam0 [[-0.39084258,  0.11035878, -0.00997172, -0.01113429,  0.30664022]] 
# cam1  [[-1.73112330e-01, -2.02276643e+00,  2.64479994e-03,  5.55261622e-03, 9.16707953e+00]]